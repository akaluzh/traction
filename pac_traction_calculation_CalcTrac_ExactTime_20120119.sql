  -- тяговый расчет, заданное время 
  procedure CalcTrac_ExactTime(CalcID in number, ExactTime in number, MakeStops in integer:= 0, vOptim in integer) is
  ...
  begin
    ...
    vStayTime:=0; 
    for c in (select * from stay_time t where t.calc=CalcId)
      loop
        vStayTime:=vStayTime+nvl(c.time_minutes,0)/60;
      end loop;
    vAverSpeed0:=(vEndPoint-vS1)/(ExactTime/60-vStayTime);  
    ...
    while (vS1<vEndPoint)
     loop
      vS2:=round(vS1+vdS,-log(10,vdS));
       ...
       -- поиск необходимой средней скорости на оставшемся участке пути 
      vAverSpeed:=(vEndPoint-vS1)/greatest((ExactTime/60-vStayTime-vTime), 0.001);
      vTime1:=0; vDist1:=0;
      for tt in (select r.startkm+0.1*r.startm Pstart, r.endkm+0.1*r.endm Pend, r.velocity
                  from velocity_limits r 
                 where r.profile = vProfileId and r.startkm+0.1*r.startm>vS2 and r.velocity<=vAverSpeed0)
        loop
          vTime1:=vTime1+1.3*(tt.pend-tt.pstart)/tt.velocity;
          vDist1:=vDist1+(tt.pend-tt.pstart);
        end loop;  
      vAverSpeed:= (vEndPoint-vS1-vDist1)/greatest((ExactTime/60-vStayTime-vTime-vTime1), 0.001);
      if vAverSpeed <=0 or vAverSpeed > 1.5*VelLimit then
          select nvl(min(vl.velocity),60) into vAverSpeed from v_velocity_limits vl where vl.calc = CalcId
   ...
   -- первичный выбор режима движения
          if (vV1<0.9*VelLimit) and (vV1< 0.8*vAverSpeed) then                                                         
            ...
            select t.s1, t.v1, t.modetype, t.time_hours, t.energy into bS1, bV1, vModeTypeId, bt1, ben1 from calctraction_output t where t.id=
                  (select max(id) from calctraction_output tt 
                                 where tt.calc=Calcid 
                                   ...
                                   and tt.s1>vS2-3 and tt.v1>vSpeedMin);
  ...
  -- корректировка режима при превышении допустимой скорости
      if (vV2>VelLimit or vV2> 1.5*vAverSpeed) and vS2>=vS_Mark then
        VelLimit:=least(VelLimit, 1.5*vAverSpeed);
        vS_Mark:=vS2;
      ...
      if vS2>vS_Mark then
        ...
        -- выход из режима торможения 
      if vV2<=0.8*least(VelLimit,1.5*vAverSpeed) and vS2>=vS_Mark then
        vModeTypeId:=pac_constant.cMotion_Type('traction');       
        if Vellimit>15 then
           vdS:=vdS0;
        else 
